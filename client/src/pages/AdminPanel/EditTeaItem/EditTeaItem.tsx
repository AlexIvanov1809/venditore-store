...
import { ICreateImage, IEditImage, Images } from "../../../models/ICoffeeItem";
import imageUpdater from "../../../helpers/imageUpdater";

const EditTeaItem = (): JSX.Element => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const { itemId } = useParams();
  const [image, setImage] = useState<Images>();
  const [data, setData] = useState<ITeaItem>();
  ...
  const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (image && data) {
      const uploadedImage = await imageUpdater(image, data);
